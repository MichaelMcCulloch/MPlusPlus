...
newtype CID = CID ((Int,Int),String) deriving (Eq, Ord, Show, Read)
...
data VarSpec = V_Spec PIdent [ArrayDimension]
  deriving (Eq, Ord, Show, Read)

data ArrayDimension = ArrDim Expr
  deriving (Eq, Ord, Show, Read)

data Type = T_Int | T_Real | T_Bool | T_Char | T_User PIdent
  deriving (Eq, Ord, Show, Read)

data FunDeclaration = D_Function PIdent ParamList Type FunBlock
  deriving (Eq, Ord, Show, Read)
...
data BasicDeclaration = BasicDec PIdent [BasicArrayDimension] Type
  deriving (Eq, Ord, Show, Read)

data BasicArrayDimension = B_ArrDim
  deriving (Eq, Ord, Show, Read)

data DataDeclaration = D_Data PIdent [ConsDecl]
  deriving (Eq, Ord, Show, Read)
...
data Location = L_Location PIdent [ArrayDimension]
  deriving (Eq, Ord, Show, Read)

data Case = C_Case CID VarList ProgStmt
  deriving (Eq, Ord, Show, Read)

data VarList = VL_List [PIdent] | VL_End
  deriving (Eq, Ord, Show, Read)
...
data IntFactor
    = IF_Expression Expr
    | IF_Size PIdent [BasicArrayDimension]
    | IF_Float Expr
    | IF_Floor Expr
    | IF_Ceil Expr
    | IF_ID PIdent ModifierList
